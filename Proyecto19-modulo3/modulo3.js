...
function agregarLibro(titulo, autor, genero, disponible) {
    ...
    setTimeout(() => {
        ...
        // Push lo puedo usar para agregar el nuevo libro al array
        biblioteca.libros.push(nuevoLibro);
        // para verificar que se agregó correctamente, podemos mostrar los libros nuevamente
        console.log("Libro agregado correctamente actualizado:");
...
function actualizarDisponibilidad(titulo, nuevoEstado) {
    ...
    setTimeout(() => {
        ...
        const libroestado = biblioteca.libros.find(libro => libro.titulo === titulo);
        if (libroestado) {
            libroestado.disponible = nuevoEstado;
            console.log(`Disponibilidad del libro  "${titulo}" actualizada a: ${nuevoEstado ? 'Disponible' : 'Prestado'}`);
        }else {
            console.log(`Libro con título "${titulo}" no disponible o encontrado.`);
        }
    }, 3000);
}
